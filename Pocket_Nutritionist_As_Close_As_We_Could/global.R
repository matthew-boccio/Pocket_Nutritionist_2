...
#calorie needs by age, gender, and exercise level

male_calories = read.csv('Male_Calorie_Needs.csv', 
                         stringsAsFactors = F)
male_calories = male_calories %>%
  rename("AGE (MALE)" = "AGE",
         "SEDENTARY (MALE)" = "SEDENTARY",
         "MODERATELY ACTIVE (MALE)" = "X",
         "ACTIVE (MALE)" = "ACTIVE") %>%
  filter(`AGE (MALE)` != "")

female_calories = read.csv("Female_Calorie_Needs.csv",
                           stringsAsFactors = F)
female_calories = female_calories %>%
  rename("AGE (FEMALE)" = "AGE",
         "SEDENTARY (FEMALE)" = "SEDENTARY",
         "MODERATELY ACTIVE (FEMALE)" = "MODERATELY.ACTIVE",
         "ACTIVE (FEMALE)" = "ACTIVE")

all_calorie_needs = cbind(male_calories, female_calories)
...
food_choose_test = food_full %>%
  filter(Food == "DAIRY AND EGGS"
         & Type == "CHEESE"
         & Preparation == "BLUE, , "
         & Additions == ", , ")
food_choose_test = food_choose_test %>%
  group_by(Food) %>%
  summarize(Avg_Total_Calories_per_100g = round(mean(Calories)),
            Calories_of_Carbs = round(mean(Total_Carbs_.g.) * 4),
            Calories_of_Sugar = round(mean(Total_Sugar_.g.) * 4),
...
placeholder = food_choose_test %>%
  filter(Food == "NOTHING")
placeholder_test = rbind(placeholder, food_choose_test)


add_choose_to_compare_test = rbind(food_compare_base, food_choose_test)

comparer_test = add_choose_to_compare %>%
  filter(Food == "DAIRY AND EGGS"
         | Food == "SELECTED FOOD")
comparer_test = comparer_test %>%
...
comparer_test_graph = ggplot(comparer_test, aes(x = Calorie_Type, y = Calorie_Amount)) + 
  geom_col(aes(fill = Food), position = 'dodge') + coord_flip() + ylim(0, 900) +
  geom_text(aes(label = Calorie_Amount), position = position_nudge(x = 0, y = 1))
comparer_test_graph

food_for_pie_chart = food_choose %>%
